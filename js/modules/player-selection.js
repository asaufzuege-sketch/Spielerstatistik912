...
App.playerSelection = {
  ...
  render() {
    ...
    // Prüfe aktuelles Team
    const currentTeam = App.teamSelection ? App.teamSelection.currentTeam : 1;
    
    // Team 1: Vordefinierte Spieler + 5 manuelle Felder
    if (currentTeam === 1) {
      ...
      // Team 2 & 3: Nur 30 manuelle Eingabefelder
      const customSelected = App.data.selectedPlayers;
      ...
      for (let i = 0; i < 30; i++) {
        ...
        li.innerHTML = `
          <label class="custom-line" style="display:flex;align-items:center;gap:8px;width:100%;" for="${chkId}">
            <input id="${chkId}" type="checkbox" class="custom-checkbox" ${pre ? "checked" : ""} style="flex:0 0 auto">
            <input type="text" class="custom-num" inputmode="numeric" maxlength="3" placeholder="Nr." value="${App.helpers.escapeHtml(pre?.num || "")}" style="width:56px;flex:0 0 auto;padding:6px;border-radius:6px;border:1px solid #444;">
            <input type="text" class="custom-name" placeholder="Spielername eingeben" value="${App.helpers.escapeHtml(pre?.name || "")}" style="flex:1;min-width:0;border-radius:6px;border:1px solid #444;padding:6px;">
          </label>`;
        this.container.appendChild(li);
      }
    }
  ...
  handleConfirm() {
    try {
      ...
      if (currentTeam === 1) {
        // Team 1: Vordefinierte + Custom Spieler
        ...
        // Team 2 & 3: Nur Custom Felder
        const customLis = Array.from(this.container.querySelectorAll("li"));
        customLis.forEach(li => {
          const chk = li.querySelector(".custom-checkbox");
          const numInput = li.querySelector(".custom-num");
          const nameInput = li.querySelector(".custom-name");
